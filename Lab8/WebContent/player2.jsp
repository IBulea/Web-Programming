...
<body>
    ...
    try {
		...
		ResultSet rsa = conn.createStatement().executeQuery("select * from game2"); 
		ResultSet rr = conn.createStatement().executeQuery("select * from game2"); 
		int count =0;
		while(rr.next())
		{
		...
		if(count==0)
			out.println("GAME OVER! PLAYER 2 WINS!!!");
		%>
		<div class="topic">
		<table>
		<%
		...
		%>
			<tr>
				<td> <% if(rsa.getString("1").equals("0") || rsa.getString("1").equals("1"))
					out.println("~");
				if(rsa.getString("1").equals("X"))
					out.println("X");
				if(rsa.getString("1").equals("*"))
					out.println("*");
				%></td>
    ...
    %>
	<form action="Bombard2" method="post">
